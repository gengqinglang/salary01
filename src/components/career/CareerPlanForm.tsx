...
const CareerPlanForm: React.FC<CareerPlanFormProps> = ({
  job,
  level,
  workStatus,
  currentIncome,
  ...
  // 检查是否有完整的职业信息
  const hasCompleteJobInfo = job && level && currentIncome && careerOutlook;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 根据不同状态显示不同的空状态卡片 */}
      {isRetired && !retirementIncome ? (
        <Card className="bg-gray-50 border-dashed border-2 border-gray-300 mt-6">
      ...
      ) : isNotWorkingFuture && (!job || !level || !startWorkAge) ? (
        <Card className="bg-gray-50 border-dashed border-2 border-gray-300 mt-6">
          <CardContent className="p-8 text-center">
            <div className="mb-4">
              <TrendingUp className="w-16 h-16 text-gray-400 mx-auto" />
